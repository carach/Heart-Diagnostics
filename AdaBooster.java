...
public class AdaBooster implements Hypothesis<Boolean> {
    ...
    public void updateWeights (Hypothesis h)
    {
        if (h == null)
            return;       
        if (epsilon >=0.5) {
            System.out.println("This learner is too weak...");
            return;
        }
        
        double alpha = 0.5 * Math.log((1 - epsilon)/epsilon);
        for (DataPoint<Boolean> dp: weights.keySet()) {
            double lastWeight = weights.get(dp);
            
            if (dp.y == h.classify(dp))   // correctly classified
                weights.replace(dp, lastWeight * Math.pow(Math.E, - alpha) / (2 * Math.pow( epsilon * ( 1 - epsilon), 0.5)));
            else           
                weights.replace(dp, lastWeight * Math.pow(Math.E, alpha) / (2 * Math.pow( epsilon * (1 - epsilon), 0.5)));

        }
    }
    /* select a tree within hypothesis space with a minimium weighted error.
    * 
    * @param Dataset
    * return the root of the select tree
    * also update epsilon
    */
    public Hypothesis selectHypothesis(ArrayList<Hypothesis> hypothesisSpace, Dataset<Boolean> trainSet)
    {
        double minError = 1;
    ...
    public void boost(ArrayList<Hypothesis> hypothesisSpace, Dataset<Boolean> trainSet, Dataset<Boolean> testSet, int times)
    {
        int counter = 1;
